...
from data.stl_10.stl_10 import STL10Dataset, get_dataloader
...
def plot_metrics(
    ...
    plt.title(f'Training and Validation Losses on {task} Task')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{task}_losses.png'), dpi=300)
    plt.close()
    ...
    plt.title(f'Other Losses, Gradient Norm Over Time on {task} Task')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{task}_details.png'), dpi=300)
    plt.close()
...
def main() -> None:
    ...
    task = 'STL-10'
    train_dataset = STL10Dataset(split='train')
    val_dataset = STL10Dataset(split='test')
    ...
    if main_process:
        print('Train Loader Example Dimensions:')
        ...
        print('\nValidation Loader Example Dimensions:')
    ...
    if main_process:
        ...
        msg = f"Lyla: We'll be training the {models_str} {grmr} on the {task} task with"
        if world_size > 1:
            print(
    ...
    for _ in range(num_epochs):
        for step, vec in enumerate(train_loader):
            vec = vec.to(device)
            for model_name in models:
                experiment = experiments[model_name]
                train_metrics = experiment.step(vec, vec) # TODO: Change to only take in vec after full migration
                ...
                for model_name, experiment in experiments.items():
                    ...
                    if main_process:
                        ...
                        if val_loss < best_val_losses[model_name]:
                            ...
                            checkpoint_filename = f'{model_name}-{task}-chkpt-step{step}-{timestamp}.safetensors'
                            ...
                            if patient_counters[model_name] >= patience:
                                ...
                                if main_process:
                                    ...
                                    if any(val_losses.values()):
                                        ...
                                        plt.title(
                                            f'Validation Losses for All Models on {task} Task',
                                            fontsize=14,
                                        )
                                        plt.grid(
    ...
    if main_process:
        for model_name in args.models:
            if model_name in best_checkpoints:
                ...
                with open(training_details, 'w') as f:
                    f.write(
                        f'Training completed for {model_name} on {task} at: {datetime.now()}\n'
                    )
        ...
        if any(val_losses.values()):
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join('plots', f'2L-{task}_all_losses.png'),
                dpi=300,
            )
            plt.close()
        else:
