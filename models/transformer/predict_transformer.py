...
def main():
    ...
    np.savetxt("predictions_tr.txt", predicted_states[:5, :, 500].cpu().numpy())
    np.savetxt(
        "ground_truths_tr.txt",
        test_targets[:5, : predicted_states.shape[1], 500].cpu().numpy(),
    )
    print(
        "Predictions and ground truths saved to 'predictions_tr.txt' and 'ground_truths_tr.txt' respectively."
    )
