...
class Mamba2Config:
    ...
    d_state: int = 128
    d_conv: int = 4
    conv_init: Optional[float] = None  # TODO: Specify type as float or None if it's a scalar value
    expand: int = 2
    headdim: int = 64
    d_ssm: Optional[int] = None  # TODO: If not None, only apply SSM to this many dimensions (rest is gated MLP)
    ngroups: int = 1
    A_init_range: Tuple[int, int] = (1, 16)  # TODO: Specify type as Tuple[int, int]
    ...
    dt_limit: Tuple[float, float] = (0.0, float('inf'))  # TODO: Specify type as Tuple[float, float]
    bias: bool = False
    conv_bias: bool = True
    chunk_size: int = 256
    use_mem_eff_path: bool = True
    layer_idx: Optional[int] = None  # TODO: Specify type as Optional[int]
    process_group: Optional[Any] = None  # TODO: Use Any or a specific type if known
    ...
    dropout: float = 0.25
    loss_fn: Optional[Any] = None  # TODO: Specify the type if it's a callable or similar
    max_len: int = 1000
    d_out: int = 29
...
class Mamba2(nn.Module):
    def __init__(self, config: Mamba2Config):
        ...
        self.world_size = 1 if config.process_group is None else config.process_group.size()
        self.local_rank = 0 if config.process_group is None else config.process_group.rank()

        self.d_inner = (self.expand * self.d_model) // self.world_size
        assert self.d_inner * self.world_size == self.expand * self.d_model

        self.headdim = config.headdim
        self.d_ssm = self.d_inner if config.d_ssm is None else config.d_ssm // self.world_size
        ...
        A = torch.empty(self.nheads, dtype=torch.float32, device=config.device).uniform_(*config.A_init_range)
        A_log = torch.log(A).to(dtype=config.dtype)
        self.A_log = nn.Parameter(A_log)
        self.A_log._no_weight_decay = True

        # D "skip" parameter
        self.D = nn.Parameter(torch.ones(self.d_ssm if self.D_has_hdim else self.nheads, device=config.device))
        ...
        if self.process_group is None:
            self.out_proj = nn.Linear(self.d_inner, config.d_out, bias=config.bias, **factory_kwargs)
        else:
            self.out_proj = RowParallelLinear(self.d_inner * self.world_size, config.d_out, bias=config.bias,
