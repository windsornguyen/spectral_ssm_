...
def apply_stu(
    ...
    print(f'Input params before compute_x_tilde : {inputs.shape}')
    x_tilde = torch_stu_utils.compute_x_tilde(inputs, eigh)
    ...
    delta_ar_u = torch_stu_utils.compute_ar_x_preds(m_u, inputs)
    ...
    return torch_stu_utils.compute_y_t(m_y, delta_phi + delta_ar_u)
...
class STU(nn.Module):
    ...
    def __init__(
        ...
        self.d_out = d_out
        self.eigh = torch_stu_utils.get_top_hankel_eigh(input_len, num_eigh)
        ...
        self.m_u = nn.Parameter(
            torch_stu_utils.get_random_real_matrix(
                (d_out, d_out, auto_reg_k_u), self.m_x_var
            )
        )
...
class Architecture(nn.Module):
    ...
    def forward(self, inputs, is_training=True):
        ...
        for i in range(self.num_layers):
            ...
            x = self.batch_norms[i](x.contiguous())
