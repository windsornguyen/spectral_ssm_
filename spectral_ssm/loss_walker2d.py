...
def loss_fn(outputs: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        for i in range(outputs.shape[1]):
            ...
            if i in (0, 1):  # coordinates of the torso
                loss = loss / 2.5
                print(f'Index {i}, Coordinate Loss Scale /2.5: {loss.mean().item()}')
            elif i in (2, 3, 4, 5, 6, 7, 8):  # angles of the torso and joints
                loss = loss / 0.5
                print(f'Index {i}, Angle Loss Scale /0.5: {loss.mean().item()}')
            elif i in (9, 10):  # coordinate velocities of the torso
                loss = loss / 2
                print(f'Index {i}, Coordinate Velocity Loss Scale /2: {loss.mean().item()}')
            elif i in (11, 12, 13, 14, 15, 16, 17):  # angular velocities of the torso and hinges
                loss = loss / 2.5
