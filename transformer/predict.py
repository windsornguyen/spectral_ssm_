...
def predict_and_plot(model, input_trajectories, target_trajectories, num_trajectories, init_steps, pred_steps, ar_steps):
    ...
    for traj_idx in range(num_trajectories):
        ...
        axs[traj_idx, 0].plot(range(time_steps), target_trajectories[traj_idx, :time_steps, 0], 'r--', label=f'Ground Truth {traj_idx+1}', linewidth=2)
        axs[traj_idx, 0].plot(range(time_steps), predicted_states[traj_idx, :, 0], 'b-', label=f'Predicted {traj_idx+1}', linewidth=2)
        ...
        axs[traj_idx, 1].plot(range(time_steps), smooth_curve(trajectory_losses[traj_idx]), 'g-', label=f'Loss {traj_idx+1}', linewidth=2)
