...
def load_data(file_path):
    while True:
        try:
            ...
            file_path = input(
                f'Please enter a valid file path for {file_path}: '
            )
        except ValueError:
            colored_print(
                f'Invalid data format in file: {file_path}', Colors.FAIL
            )
            file_path = input(
                f'Please enter a valid file path for {file_path}: '
            )
...
def plot_data(
    data_list, time_steps_list, labels, title, xlabel, ylabel, ax=None
):
    if ax is None:
...
def main():
    ...
    while True:
        ...
        choice = get_user_input(
            f'{Colors.BOLD}Enter your choice (1-6):{Colors.ENDC} '
        )
        ...
        if choice in ['1', '2', '3', '5', '6']:
            ...
            num_train_plots = get_user_input(
                f'{Colors.BOLD}Enter the number of train plots:{Colors.ENDC} ',
                int,
            )
            train_data_list = []
            ...
            for i in range(num_train_plots):
                ...
                if data is not None:
                    ...
                    label = input(
                        f'Enter the legend name for train plot {i+1}: '
                    )
                    train_labels.append(label)
                    train_time_steps_list.append(np.arange(len(data)))

            num_val_plots = get_user_input(
                f'{Colors.BOLD}Enter the number of val plots:{Colors.ENDC} ',
                int,
            )
            val_data_list = []
            val_time_steps_list = []
            ...
            plot_time_steps_list.append(
                train_time_steps_list + val_time_steps_list
            )
            plot_labels.append(train_labels + val_labels)
            title = input(
                'Enter the title for the training and validation plot: '
            )
    ...
    if same_plot.lower() == 'y':
        ...
        for data_list, time_steps_list, labels, title, xlabel, ylabel in zip(
            ...
            plot_data(
                data_list, time_steps_list, labels, title, xlabel, ylabel, ax
            )
        plt.tight_layout()
        ...
        if num_subplots == 1:
            data_list, time_steps_list, labels, title, xlabel, ylabel = (
                plot_data_list[0],
                plot_time_steps_list[0],
                plot_labels[0],
                plot_titles[0],
                plot_xlabels[0],
                plot_ylabels[0],
            ...
            filename = (
                input(
                    f'Enter the filename for plot 1 (default: {default_filename}): '
                )
                or default_filename
            )
            plt.savefig(os.path.join(save_path, filename), dpi=300)
            colored_print(
                f'Plot saved at: {os.path.join(save_path, filename)}',
                Colors.OKGREEN,
            )
        else:
            for i, (
